...
std::vector<float> Steer(  std::vector<float> x_nearest , std::vector<float> x_rand, float eta){
  ...
  if (Norm(x_nearest,x_rand)<=eta){
    // 随机点x_rand与x_nearst距离小于步长eta，那么新拓展的点就是x_rand
    x_new=x_rand;
  }
  else{
    float m=(x_rand[1]-x_nearest[1])/(x_rand[0]-x_nearest[0]);

    x_new.push_back(  (sign(x_rand[0]-x_nearest[0]))* (   sqrt( (pow(eta,2)) / ((pow(m,2))+1) )   )+x_nearest[0] );// x_new = x_0+|eta/cos(theta)|*sign(x1-x0)
    x_new.push_back(  m*(x_new[0]-x_nearest[0])+x_nearest[1] );// y_new=y0+(x_new-x0)*tan(theta)
